    ...
    if (selected_year and selected_level and selected_subject and selected_paper):
        ...
        if num_files == 1 or combined_df['year'].nunique() == 1:
            ...
            return html.Div([
                dbc.Row([
                    dbc.Card([
                        dbc.CardBody([
                            html.H2("📊 Single Paper Analysis", style=SECTION_HEADER_STYLE),
                            #html.P("Explore linguistic patterns in Scottish exam papers", className="lead")
                        ])
                    ], style=CARD_STYLE),
                ]),
                ...
                dbc.Row([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("📈 Basic Metrics", style=SECTION_HEADER_STYLE),
                            dbc.Row([
                                dbc.Col([
                                    html.Div([
                                        html.P(f"📏 Average Word Count: {avg_word_count:.2f}", className="h5"),
                                        html.P(f"❓ Total Questions: {num_questions}", className="h5"),
                                    ], style=STATS_CARD_STYLE)
                                ], width=6),
                                
                                dbc.Col([
                                    html.Div([
                                        html.P("📊 Readability Scores", className="h5"),
                                        html.Ul([
                                            html.Li(f"Coleman-Liau: {stats['coleman_liau']['mean']:.2f}"),
                                            html.Li(f"Flesch-Kincaid: {stats['flesch_kincaid']['mean']:.2f}"),
                                            html.Li(f"Gunning Fog: {stats['gunning_fog']['mean']:.2f}"),
                ...
                dbc.Row([
                    dbc.Card([
                        dbc.CardBody([
                            dbc.ButtonGroup([
                                dbc.Button("📥 Download CSV", id="download-csv-btn", color="primary", className="me-2"),
                                dcc.Download(id="download-csv"),
                                dbc.Button("📊 Download Excel", id="download-excel-btn", color="success"),
                ...
                dbc.Row([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("🎯 Intent Breakdown", style=SECTION_HEADER_STYLE),
                            dcc.Graph(figure=intent_fig),
                            dbc.Accordion([
                                dbc.AccordionItem(
                                    intent_description,
                                    title="ℹ️ About Intent Classification"
                                )
                ...
                dbc.Row([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("📉 Sentiment Analysis", style=SECTION_HEADER_STYLE),
                            dcc.Graph(figure=sentiment_fig),
                            dbc.Accordion([
                                dbc.AccordionItem(
                                    sentiment_description,
                                    title="ℹ️ About Sentiment Analysis"
                                )
                            ], start_collapsed=True)
                        ])
                    ], style=CARD_STYLE),
                ...
                dbc.Row([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("📏 Question Length", style=SECTION_HEADER_STYLE),
                            dcc.Graph(figure=sentence_length_fig),
                            dbc.Accordion([
                                dbc.AccordionItem(
                                    question_length_description,
                                    title="ℹ️ About Question Length"
                                )
                            ], start_collapsed=True)
                        ])
                    ], style=CARD_STYLE),
                ...
                dbc.Row([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("🌍 Named Entities", style=SECTION_HEADER_STYLE),
                            DashWordcloud(
                                id="cloud",
                                list=topics_result,
                                ...
                                hover=True),  # Keep existing config
                            dbc.Accordion([
                                dbc.AccordionItem(
                                    named_entity_description,
                                    title="ℹ️ About Named Entities"
                                )
                            ], start_collapsed=True),
                            html.H4("", id="report"),
                ...
                dbc.Row([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("📚 Readability Analysis", style=SECTION_HEADER_STYLE),
                            dcc.Graph(figure=complexity_fig),
                            dbc.Accordion([
                                dbc.AccordionItem(
                                    readability_explanation,
                                    title="ℹ️ About Readability Metrics"
                                )
                            ], start_collapsed=True)
                        ])
                    ], style=CARD_STYLE),
                ...
                dbc.Row([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("📄 Full Question List", style=SECTION_HEADER_STYLE),
                            dash_table.DataTable(
                                data=questions_data_to_display,
                                columns=questions_columns_to_display,
                ...
                dbc.Row([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("📚 Weighted Comparative Analysis", style=SECTION_HEADER_STYLE),
                            dcc.Graph(id="ca_report", style={"display": "none"}),
                            dbc.Accordion([
                                html.Script("renderKatex();"),
                                dbc.AccordionItem(
                                    wcs_visualisation,
                                    title="ℹ️ About Weighted Comparative Analysis"
                                )                                
                            ], start_collapsed=True)
                        ]),
                        dbc.CardBody([
                            html.H4("📚 Linear Regression Analysis", style=SECTION_HEADER_STYLE),
                            dcc.Graph(id="ca_linreg", style={"display": "none"}),
                            dbc.Accordion([
                                html.Script("renderKatex();"),
                                dbc.AccordionItem(
                                    linear_regression_description,
                                    title="ℹ️ About Linear Regression Analysis"
                                )                                
                            ], start_collapsed=True)
                        ]),
                        dbc.CardBody([
                            html.H4("📚 Trend Component Analysis", style=SECTION_HEADER_STYLE),
                            dcc.Graph(id="ca_ts1", style={"display": "none"}),
                            dbc.Accordion([
                                html.Script("renderKatex();"),
                                dbc.AccordionItem(
                                    un_trend_desc,
                                    title="ℹ️ About Trend Component Analysis"
                                )                                
                            ], start_collapsed=True)
                        ])
                    ], style=CARD_STYLE),
        ...
        return html.Div([
            dbc.Row([
                dbc.Card([
                    dbc.CardBody([
                        html.H2("📈 Multiple Papers Trend Analysis", style=SECTION_HEADER_STYLE),
                        html.P("Longitudinal analysis across multiple exam papers", className="lead")
                    ])
                ], style=CARD_STYLE),
            ]),
            
            # Statistics Cards
            *[dbc.Row(dbc.Card(dbc.CardBody(plot), style=CARD_STYLE)) for plot in stat_plots],
            ...
            dbc.Row([
                dbc.Card([
                    dbc.CardBody([
                        html.H4("📉 Sentiment Trend", style=SECTION_HEADER_STYLE),
                        dcc.Graph(figure=sentiment_fig),
                        dbc.Accordion([
                            dbc.AccordionItem(
                                sentiment_description,
            ...
            dbc.Row([
                dbc.Card([
                    dbc.CardBody([
                        html.H4("📏 Average Question Length", style=SECTION_HEADER_STYLE),
                        dcc.Graph(figure=sentence_length_fig),
                        dbc.Accordion([
                            dbc.AccordionItem(
                                question_length_description,
            ...
            dbc.Row([
                dbc.Card([
                    dbc.CardBody([
                        html.H4("📚 Readability Trends", style=SECTION_HEADER_STYLE),
                        dcc.Graph(figure=complexity_fig),
                        dbc.Accordion([
                            dbc.AccordionItem(
                                readability_explanation,
    ...
    return html.Div([
        ...
        html.P(
            """This dashboard allows for one to interact with analysis results on Scottish exam papers.
                   Currently a work in progress. Note that 'old style' levels have been converted to 
                   their modern CfE equivalent to simplify analysis for the end user (that's you!)."""
        )])
...
def update_dropdowns(selected_year, selected_level, selected_subject, selected_paper):
    available_years = set(directory_info.keys())
    ...
    for year, levels in directory_info.items():
        if selected_year and year != selected_year:
            continue  # Filter by selected year

        for level, papers in levels.items():
            if selected_level and level != selected_level:
                continue  # Filter by selected level
            
            available_levels.add(level)  # Collect valid levels

            for paper, subjects in papers.items():
                if selected_paper and paper != selected_paper:
                    continue  # Filter by selected paper
                
                available_papers.add(paper)  # Collect valid papers

                for subject in subjects:
                    if selected_subject and subject != selected_subject:
                        continue  # Filter by selected subject
                    
                    available_subjects.add(subject)  # Collect valid subjects

    # Convert sets to dropdown option format
    year_options = [{"label": y, "value": y} for y in sorted(available_years)]
    level_options = [{"label": l, "value": l} for l in sorted(available_levels)]
    subject_options = [{"label": s, "value": s} for s in sorted(available_subjects)]
    paper_options = [{"label": p, "value": p} for p in sorted(available_papers)]

    return year_options, level_options, subject_options, paper_options
